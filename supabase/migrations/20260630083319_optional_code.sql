-- Discount code is now optional. A lead can register without one; if a code IS
-- supplied it must still be one of the valid, unused codes (single-use).

alter table public.leads alter column discount_code drop not null;
...
begin
  ...
  -- a code was supplied -> it must be valid & unused
